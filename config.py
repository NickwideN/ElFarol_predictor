...
У каждого предиктора есть условие. Каждый день, когда условие предиктора выдает правильное решение (совпадающее с is_day_success в config.py), день считается успешным для предиктора.
...
А представьте, что каждый день этот процент меняется то больше 50, то меньше 50, тогда предиктор каждый день будет выдавать разные значения, поэтому будет не помтоянен, будет ошибаться и в наборе предикторов его заменят, из за чего на графике пройзойдет скачок.

Поэтому разрешать менять условие на противоположное разумно только тем предикторам, процент успешных у которого меньше 50%
"""